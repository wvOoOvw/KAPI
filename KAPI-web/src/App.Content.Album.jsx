          ...
          return <div>
            {
              breakpointCols ?
                ...
                >
                  {
                    album.map((i) => <Album key={i._id} album={i} />)
                  }
                </Masonry>
                : null
            }
...
function App() {
  ...
  const [albumFilter, setAlbumFilter] = React.useState({ name: new URLSearchParams(new URL(window.location.href).search).get('search') || '', latest: false })
  ...
  const onFetchAlbum = async () => {
    ...
    await Fetch.json('/api/app/album/find/list', { filter: { ...albumFilter, status: 1, latest: undefined }, seed: seed, skip: 0, limit: 10 })
      .then(res => {
        setAlbum(res.data)
  ...
  const onFetchAlbumScroll = async () => {
    ...
    await Fetch.json('/api/app/album/find/list', { filter: { ...albumFilter, status: 1, latest: undefined }, seed: albumSeed, skip: album.length, limit: 10 })
      .then(res => {
